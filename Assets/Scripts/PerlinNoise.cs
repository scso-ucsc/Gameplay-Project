...
public class PerlinNoise : MonoBehaviour
{
    ...
    void Start()
    {
        xOffset = Random.Range(0f, 999f); //Randomly generating offset
    ...
    private Color generateColour(int x, int y){ //Generating and returning colour for each pixel
        float xCoordinate = (float) x / textureWidth * scale + xOffset; //Determining x-coordinate based on overall scale
        float yCoordinate = (float) y / textureHeight * scale + yOffset; //Determining y-coordinate based on overall scale
